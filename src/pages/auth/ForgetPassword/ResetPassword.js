...
import { reset, onResetPasswordFP } from 'redux/actions';
  ...
  const onSubmit = async (data, e) => {
    ...
    if (Object.keys(errors).length === 0) {
      let requestPayload = {
        ...
        resetPasswordToken: firstStep?.resetPasswordToken,
      };

      await dispatch(onResetPasswordFP(requestPayload));
    }
  };

  useEffect(() => {

    if (common?.error?.isAccountLock) {
      // // navigate to lock account page
    } else if (common.data?.success) {
      handleNext({
        firstStep: {
          ...firstStep,
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <Box sx={{ mb: { xs: 3, xl: 4 } }}>
        <ReactHookInput
          ...
          endAdornment={
            values?.showConfirmPassword ? <VisibilityOffIcon /> : <Visibility />
          }
