...
const generateRoutes = (structure) => {
  ...
  if (isAuthenticated && authorizedStructure) {
    dynamicRoutes.push(
      ...routesGenerator(
        isAuthenticated,
        authorizedStructure,
        RouteStructure.AUTHORIZED,
        isAuthenticated ? userRole : null,
      ),
    );
  }

  if (!isAuthenticated && unAuthorizedStructure) {
    dynamicRoutes.push(
      ...routesGenerator(
        isAuthenticated,
        unAuthorizedStructure,
        RouteStructure.UNAUTHORIZED,
      ),
    );
  }
  ...
  if (publicStructure) {
    dynamicRoutes.push(
      ...routesGenerator(null, publicStructure, RouteStructure.PUBLIC),
    );
  }
...
const routesGenerator = (
  isAuthenticated = false,
  routeSet = {},
  type = RouteStructure.ANONYMOUS,
  userRole,
) => {
  const generatedRoutes = [];
  const {fallbackPath = ''} = routeSet || {};

  const isAnonymous = type === RouteStructure.ANONYMOUS;
  const isAuthorized = type === RouteStructure.AUTHORIZED;
  const isPublic = type === RouteStructure.PUBLIC;
